...
namespace Lab3_LS_ASAP
{
    class CLS_ASAP
    {
        ...
        public void File_Load(string filestr)
        {
            ...
            //step 5 чтение таблицы смежности
            buff = "";
            buff = file.ReadLine();          
        ...
        private void WriteSteps()
        {
            ...
            for (int i = 0; i < List_chains.Count; i++)
            {
                List_operations_on_steps[index_step][1].Add(List_chains[i][0]);
            }
            ...
            while (true)
            {
                ...
                // перемещение всех нераспланированных на этом шаге операций в левую часть списка
                List_operations_on_steps[index_step][0] = minus(ready_operations, steps[index_step]);
                ...
                for (int i = 0; i < steps[index_step].Count; i++)
                {
                    ...
                    {
                        if (List_chains[k].Contains(steps[index_step][i]))
                        {
                            int index_operation = List_chains[k].IndexOf(steps[index_step][i]);
                ...
                for (int i = 0; i < List_chains_ready_for_step.Count; i++)
                {
                    ...
                    if (index_false != -1)
                    {
                        ...
                        {
                            List_operations_on_steps[index_step][1].Add(value_index_false);
                        }
                    }
                }                
            }
